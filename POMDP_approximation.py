class POMDP:
    def __init__(self,n,init,lose,losing_obs,k,p,Delta,mu):
...
#
    def approximation(self,belief):
        ...
        for i in range(self.nb_state):
            qi = belief[i]
            k = int(qi/self.approx)
            maxi = min(1,(k+1)*self.approx)                                                     # on ne veut pas dépasser la proba de 1
            if (abs(k*self.approx - qi) < abs(maxi - qi)):
                approx.append(k*self.approx)
                dist.append((abs(k*self.approx - qi),i))
            else:
                approx.append(maxi)
                dist.append((maxi,i))
        s = 0
            ...
            if (s < 1):
                ...
                while (nb != 0):                                                            # de même pour -nb
                    ...
                    if approx[i] > belief[i]:
                        ...
                        nb += 1
                    c += 1
        return(approx)                                                                      # c'est bien une distribution de proba car mu = 1/k et nb est le nombre d'élément à changer dans un sens pour avoir une distribution de proba
